...
export class QuickStartNetwork extends ModalAction {
  ...
  validateIpv6 = (rule, value) => {
    if (!value || ipValidate.isIpv6(value)) {
      return Promise.resolve();
    }
  ...
  onOk = async (values, containerProps, callback) => {
    ...
    try {
      ...
      Notify.success(
        t('Create network successfully, instance: {name}.', {
          name: networkName,
        })
      );
  ...
  prepareSubnetData = (values) => {
    ...
    return {
      ...
      enable_dhcp: true,
      dns,
      ...(gatewayIp ? { gateway_ip: gatewayIp } : {}),
    };
  };
